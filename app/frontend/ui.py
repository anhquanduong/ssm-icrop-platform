...
if "async_runner" not in st.session_state:
    st.session_state.async_runner = AsyncSimulationRunner()
if "lat_key" not in st.session_state:
    st.session_state["lat_key"] = 21.0285
if "lon_key" not in st.session_state:
    st.session_state["lon_key"] = 105.8542
if "current_preset" not in st.session_state:
    st.session_state.current_preset = "Custom Coordinates"
if "fertilizer_rounds" not in st.session_state:
    st.session_state.fertilizer_rounds = []
if "irrigation_rounds" not in st.session_state:
    st.session_state.irrigation_rounds = []
if "drainage_rounds" not in st.session_state:
    st.session_state.drainage_rounds = []
if "som_key" not in st.session_state:
    st.session_state["som_key"] = 2.5
if "pawc_key" not in st.session_state:
    st.session_state["pawc_key"] = 150.0
if "depth_key" not in st.session_state:
    st.session_state["depth_key"] = 1200
    ...
    with tab_login:
        ...
        if st.button("Log In", use_container_width=True):
            ...
            if success and payload:
                ...
                st.session_state.workplace = payload["workplace"]
                st.session_state.is_verified = payload["is_verified"]
...
with st.sidebar:
    ...
    if "selected_crop_profile" not in st.session_state:
        st.session_state["selected_crop_profile"] = "Default: Maize (BOKU)"
        
    if st.session_state["selected_crop_profile"] not in profile_options:
        st.session_state["selected_crop_profile"] = "Default: Maize (BOKU)"
        
    selected_index = profile_options.index(st.session_state["selected_crop_profile"])
    ...
    st.session_state["selected_crop_profile"] = selected_option
    ...
    if selected_option == "➕ Create New Crop Profile...":
                ...
                if not new_crop_name.strip():
                    ...
                    if new_crop_name.strip().lower() in existing_names:
                        ...
                        try:
                            ...
                            if is_public_flag:
                                newly_created_label = f"🌐 Public: {new_crop_name.strip()} (Shared by {st.session_state.username})"
                            else:
                                newly_created_label = f"🔒 Private: {new_crop_name.strip()} (My Profile)"
                            st.session_state["selected_crop_profile"] = newly_created_label
                            st.rerun()
                        except Exception as save_err:
                            st.error(f"Failed to register new crop profile: {save_err}")
                            
        default_lat = st.session_state.get("lat_key", 21.0285)
        default_lon = st.session_state.get("lon_key", 105.8542)
    else:
        ...
        active_lat = st.session_state.get("lat_key", 21.0285)
        active_lon = st.session_state.get("lon_key", 105.8542)
        ...
        if preset != preset_val:
            if preset == "Gerasdorf, Austria (Wheat/Maize Study)":
                st.session_state["lat_key"], st.session_state["lon_key"] = 48.2830, 16.4670
            elif preset == "Iowa, USA (Maize Belt)":
                st.session_state["lat_key"], st.session_state["lon_key"] = 42.0300, -93.6310
            elif preset == "Mekong Delta, Vietnam (Rice/Crops)":
                st.session_state["lat_key"], st.session_state["lon_key"] = 10.0330, 105.7830
            elif preset == "Custom Coordinates":
                st.session_state["lat_key"], st.session_state["lon_key"] = 21.0285, 105.8542
            st.rerun()
            
        default_lat = st.session_state["lat_key"]
        default_lon = st.session_state["lon_key"]
...
with col_left:
    ...
    if "📁 Upload My Own Weather" in weather_source:
        ...
        if "lat_key" not in st.session_state:
            st.session_state["lat_key"] = 21.0285  # Default Lat
        if "lon_key" not in st.session_state:
            st.session_state["lon_key"] = 105.8542 # Default Lon
        ...
        folium.Marker(
            location=[st.session_state["lat_key"], st.session_state["lon_key"]],
            popup=f"Active Location: {st.session_state['lat_key']:.4f}, {st.session_state['lon_key']:.4f}",
        ...
        map_data = st_folium(
            m,
            center=[st.session_state["lat_key"], st.session_state["lon_key"]],
            zoom=6,
            width=700,
            height=400,
        ...
        if map_data and map_data.get("last_clicked"):
            ...
            if new_lat != st.session_state["lat_key"] or new_lon != st.session_state["lon_key"]:
    ...
    with st.expander("🧪 Crop Nutrition & Fertilizer Management", expanded=False):
        ...
        if col_remove.button("➖ Remove Last Round", key="remove_fert_round_btn"):
            if len(st.session_state.fertilizer_rounds) > 0:
                st.session_state.fertilizer_rounds.pop()
                st.rerun()

        if len(st.session_state.fertilizer_rounds) == 0:
            st.info("ℹ️ No scheduled operations added yet. Click '➕ Add Round' below to configure an application event.")
        else:
            for idx, round_item in enumerate(st.session_state.fertilizer_rounds):
    ...
    with st.expander("💧 Water Management (Irrigation & Drainage)", expanded=False):
        ...
        if col_irr_add.button("➕ Add Irrigation Round", key="add_irr_round_btn"):
            st.session_state.irrigation_rounds.append(
                {"doy": 130, "amount": 0.0, "type": "Drip"}
            )
            st.rerun()

        if col_irr_remove.button("➖ Remove Last Irrigation", key="remove_irr_round_btn"):
            if len(st.session_state.irrigation_rounds) > 0:
                st.session_state.irrigation_rounds.pop()
                st.rerun()

        if len(st.session_state.irrigation_rounds) == 0:
            st.info("ℹ️ No scheduled operations added yet. Click '➕ Add Round' below to configure an application event.")
        else:
            for idx, round_item in enumerate(st.session_state.irrigation_rounds):
        ...
        if col_drn_add.button("➕ Add Drainage Setup", key="add_drn_round_btn"):
            st.session_state.drainage_rounds.append(
                {"doy": 150, "rate": 0.0, "type": "Surface Runoff"}
            )
            st.rerun()

        if col_drn_remove.button("➖ Remove Last Drainage", key="remove_drn_round_btn"):
            if len(st.session_state.drainage_rounds) > 0:
                st.session_state.drainage_rounds.pop()
                st.rerun()

        if len(st.session_state.drainage_rounds) == 0:
            st.info("ℹ️ No scheduled operations added yet. Click '➕ Add Round' below to configure an application event.")
        else:
            for idx, round_item in enumerate(st.session_state.drainage_rounds):
    ...
    with c_name:
        scenario_name = st.text_input(
            "Scenario Label",
            value=f"Run #{len(st.session_state.get('sim_history', {})) + 1}",
...
with col_right:
    ...
    with tab_sim:
        ...
        if st.session_state.is_verified == 0:
    ...
    with tab_comp:
        ...
        if not st.session_state.get("sim_history"):
            ...
            selected_runs = st.multiselect(
                ...
                options=list(st.session_state["sim_history"].keys()), 
                default=list(st.session_state["sim_history"].keys()),
                help="Select one or more runs to construct overlay comparisons."
            )
            
            if selected_runs:
                import plotly.express as px
                combined_df = pd.concat([st.session_state["sim_history"][run] for run in selected_runs])
            ...
            with st.expander("📥 Export Results & Simulation Data Logs", expanded=False):
                ...
                summary_data = []
                for label, df in st.session_state["sim_history"].items():
                    summary_data.append({
                ...
                with col_csv:
                    # CSV generation
                    csv_data = export_history_to_csv(st.session_state["sim_history"])
                    st.download_button(
                ...
                with col_xlsx:
                    ...
                    xlsx_data = export_history_to_xlsx(
                        sim_history=st.session_state["sim_history"],
                        soil_config=active_soil_config,
                        latitude=st.session_state.get("lat_key", 21.0285),
                        longitude=st.session_state.get("lon_key", 105.8542),
                        crop_name=st.session_state.get("selected_crop_profile", "Default Crop")
                    )
                    st.download_button(
    ...
    with tab_acc:
        ...
        with st.form("update_profile_form"):
            ...
            prof_name = st.text_input("Full Name", value=st.session_state.name)
            prof_work = st.text_input("Workplace / Institution", value=st.session_state.workplace)
            
            if st.form_submit_button("Update Profile Details"):
                success, msg = update_user_profile(st.session_state.user_id, prof_name, prof_work)
                if success:
                    st.session_state.name = prof_name
                    st.session_state.workplace = prof_work
                    st.success(msg)
                    st.rerun()
                else:
                    st.error(msg)
        ...
        with st.form("change_password_form"):
            ...
            if st.form_submit_button("Request Password Change Token"):
                if new_pass != confirm_pass:
                    ...
                    success, msg, payload = authenticate_secure_user(st.session_state.username, curr_pass)
                    if success:
                        # Password correct, dispatch reset/verification email link
                        req_success, req_msg = request_password_reset(st.session_state.email)
